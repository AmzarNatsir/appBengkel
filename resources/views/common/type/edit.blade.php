        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <div class="card-title">Form Edit Common Type</div>
                </div>
                <div class="card-body">
                    <form action="{{ url('common/typeUpdate', $data->id) }}" method="POST">
                    @csrf
                    {{ method_field('PUT') }}
                        <div class="row">
                            <div class="col-md-12 col-lg-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="type_name" id="type_name" class="form-control form-control-sm @error('type_name') is-invalid @enderror" value="{{ $data->type_name }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-select @error('brand_select') is-invalid @enderror" name="brand_select" id="brand_select">
                                        ...
                                        @foreach ($listBrand as $brand)
                                            @if($brand->oid_brand==$data->oid_brand)
                                            <option value="{{ $brand->oid_brand }}" selected>{{ $brand->brand_name }}</option>
                                            @else
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-select @error('model_select') is-invalid @enderror" name="model_select" id="model_select">
                                        ...
                                        @foreach ($listModel as $model)
                                            @if($model->oid_model==$data->oid_model)
                                            <option value="{{ $model->oid_model }}" selected>{{ $model->model_name }}</option>
                                            @else
                                            <option value="{{ $model->oid_model }}">{{ $model->model_name }}</option>
                                            @endif
                                        @endforeach
                                    </select>
